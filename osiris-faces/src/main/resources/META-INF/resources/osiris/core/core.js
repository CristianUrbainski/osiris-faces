(function(window) {
    ...
    if(window.Osiris) {
        Osiris.debug("Osiris already loaded, ignoring duplicate execution.");
        return;
    }
    ...
    Osiris = {
        ...
        //ajax shortcut
        ab: function(cfg, ext) {
            Osiris.ajax.AjaxRequest(cfg, ext);
        },
        ...
        cw : function(widgetConstructor, widgetVar, cfg, resource) {
            Osiris.createWidget(widgetConstructor, widgetVar, cfg, resource);
        },

        createWidget : function(widgetConstructor, widgetVar, cfg, resource) {            
            if(Osiris.widget[widgetConstructor]) {
                if(window[widgetVar])
                    window[widgetVar].refresh(cfg);                                     //ajax update
                else
                    window[widgetVar] = new Osiris.widget[widgetConstructor](cfg);  //page init
            }
            else {
                var scriptURI = $('script[src*="/javax.faces.resource/osiris.js"]').attr('src').replace('osiris.js', resource + '/' + resource + '.js'),
                cssURI = $('link[href*="/javax.faces.resource/osiris.css"]').attr('href').replace('osiris	.css', resource + '/' + resource + '.css'),
                ...
                Osiris.getScript(location.protocol + '//' + location.host + scriptURI, function() {
                    setTimeout(function() {
                        window[widgetVar] = new Osiris.widget[widgetConstructor](cfg);
                    }, 100);
                });
            }
        },
        ...
        focus : function(id, context) {
            ...
            setTimeout(function() {
                if(id) {
                    var jq = $(Osiris.escapeClientId(id));
                    ...
                    $(Osiris.escapeClientId(context)).find(selector).eq(0).focus();
                }
                else {
                    $(selector).eq(0).focus();
                }
        ...
        monitorDownload: function(start, complete) {
            if(this.cookiesEnabled()) {
                ...
                window.downloadMonitor = setInterval(function() {
                    var downloadComplete = Osiris.getCookie('Osiris.download');
                    ...
                    if(downloadComplete === 'true') {
                        ...
                        Osiris.setCookie('Osiris.download', null);
                    }
                }, 250);
            }
        },
        ...
        scrollTo: function(id) {
            var offset = $(Osiris.escapeClientId(id)).offset();
        ...
        openDialog: function(cfg) {
            ...
            dialogFrame.on('load', function() {
                ...
                if(!$frame.data('initialized')) {
                    Osiris.cw('Dialog', dialogWidgetVar, {
                        id: dialogId,
                        position: 'center',
        ...
        showMessageInDialog: function(msg) {
            if(!this.messageDialog) {
                var messageDialogDOM = $('<div id="Osirismessagedlg" class="ui-message-dialog ui-dialog ui-widget ui-widget-content ui-corner-all ui-shadow ui-overlay-hidden"/>')
                            .append('<div class="ui-dialog-titlebar ui-widget-header ui-helper-clearfix ui-corner-top"><span class="ui-dialog-title"></span>' +
                            '<a class="ui-dialog-titlebar-icon ui-dialog-titlebar-close ui-corner-all" href="#" role="button"><span class="ui-icon ui-icon-closethick"></span></a></div>' + 
                            '<div class="ui-dialog-content ui-widget-content" style="height: auto;"></div>')
                            .appendTo(document.body);

                Osiris.cw('Dialog', 'Osirismessagedialog', {
                    id: 'Osirismessagedlg', 
                    modal:true,
                    draggable: false, 
                    resizable: false,
                ...
                this.messageDialog = PF('Osirismessagedialog');
        ...
        confirm: function(msg) {
            if(Osiris.confirmDialog) {
                Osiris.confirmSource = $(Osiris.escapeClientId(msg.source));
                Osiris.confirmDialog.showMessage(msg);
            }
            else {
                Osiris.warn('No global confirmation dialog available.');
            }
        },
        ...
        RESET_VALUES_PARAM : "Osiris.resetValues",

        VIEW_STATE : "javax.faces.ViewState",

        VIEW_ROOT : "javax.faces.ViewRoot",

        CLIENT_ID_DATA : "Osiris.clientid"
    };
    ...
    Osiris.ajax.AjaxUtils = {

        encodeViewState : function() {
            var viewstateValue = document.getElementById(Osiris.VIEW_STATE).value;
            var re = new RegExp("\\+", "g");
        ...
        updateElement: function(id, content) {        
            if(id == Osiris.VIEW_STATE) {
                Osiris.ajax.AjaxUtils.updateState.call(this, content);
            }
            else if(id == Osiris.VIEW_ROOT) {
                ...
                $(Osiris.escapeClientId(id)).replaceWith(content);
            }
        },
        ...
        handleResponse: function(xmlDoc) {
            ...
            callbackParams = xmlDoc.find('extension[ln="Osiris"][type="args"]'),
        ...
        findComponents: function(selector) {
            ...
            components.each(function() {
                var element = $(this),
                clientId = element.data(Osiris.CLIENT_ID_DATA)||element.attr('id');
        ...
        idsToArray: function(cfg, type, selector) {
            ...
            if(ext) {
                ...
                for(var i = 0; i < extArr.length; i++) {
                    if(!Osiris.inArray(arr, extArr[i])) {
                        arr.push(extArr[i]);
                    }
                }
            }
            ...
            if(selector) {
                $.merge(arr, Osiris.ajax.AjaxUtils.findComponents(selector));
            }
        ...
        send: function(cfg) {
            Osiris.debug('Initiating ajax request.');

            if(cfg.onstart) {
                var retVal = cfg.onstart.call(this, cfg);
                if(retVal == false) {
                    Osiris.debug('Ajax request cancelled by onstart callback.');

                    //remove from queue
                    if(!cfg.async) {
                        Osiris.ajax.Queue.poll();
                    }
            ...
            if(cfg.formId) {
                form = $(Osiris.escapeClientId(cfg.formId));                         //Explicit form is defined
            }
            else {
                form = $(Osiris.escapeClientId(sourceId)).parents('form:first');     //look for a parent of source
            ...
            Osiris.debug('Form to post ' + form.attr('id') + '.');
            ...
            Osiris.debug('URL to post ' + postURL + '.');

            //partial ajax
            postParams.push({
                name:Osiris.PARTIAL_REQUEST_PARAM, 
                value:true
            });

            //source
            postParams.push({
                name:Osiris.PARTIAL_SOURCE_PARAM, 
                value:sourceId
            });
            ...
            if (cfg.resetValues) {
                postParams.push({
                    name:Osiris.RESET_VALUES_PARAM, 
                    value:cfg.resetValues
                });
            }

            //process
            var processArray = Osiris.ajax.AjaxUtils.idsToArray(cfg, 'process', cfg.processSelector);
            if(cfg.fragmentId) {
                processArray.push(cfg.fragmentId);
            }
            var processIds = processArray.length > 0 ? processArray.join(' ') : '@all';
            postParams.push({
                name:Osiris.PARTIAL_PROCESS_PARAM, 
                value:processIds
            });

            //update
            var updateArray = Osiris.ajax.AjaxUtils.idsToArray(cfg, 'update', cfg.updateSelector);
            if(cfg.fragmentId&&cfg.fragmentUpdate) {
                updateArray.push(cfg.fragmentId);
            }
            if(updateArray.length > 0) {
                postParams.push({
                    name:Osiris.PARTIAL_UPDATE_PARAM, 
            ...
            if(cfg.event) {
                postParams.push({
                    name:Osiris.BEHAVIOR_EVENT_PARAM, 
                    value:cfg.event
                });
                ...
                postParams.push({
                    name:Osiris.PARTIAL_EVENT_PARAM, 
                    value:domEvent
                });
            } 
            else {
            ...
            */
            if(cfg.partialSubmit && processIds.indexOf('@all') == -1) {
                ...
                if(processIds.indexOf('@none') == -1) {
                	for (var i = 0; i < processArray.length; i++) {
                        var jqProcess = $(Osiris.escapeClientId(processArray[i]));
            ...
            Osiris.debug('Post Data:' + postData);
            ...
            .done(function(data, status, xhr) {
                Osiris.debug('Response received succesfully.');
                ...
                if(parsed) {
                    ...
                    Osiris.ajax.AjaxResponse.call(this, data, status, xhr);
                }

                Osiris.debug('DOM is updated.');
            })
            .fail(function(xhr, status, errorThrown) {
                ...
                Osiris.error('Request return with error:' + status + '.');
            })
            .always(function(xhr, status) {
                ...
                Osiris.debug('Response completed.');
    ...
    Osiris.ajax.AjaxRequest = function(cfg, ext) {
        cfg.ext = ext;

        if(cfg.async) {
            Osiris.ajax.AjaxUtils.send(cfg);
        }
        else {
            Osiris.ajax.Queue.offer(cfg);
        }
    }

    Osiris.ajax.AjaxResponse = function(responseXML) {
        ...
        for(var i=0; i < updates.length; i++) {
            ...
            id = update.attr('id'),
            content = Osiris.ajax.AjaxUtils.getContent(update);
            
            Osiris.ajax.AjaxUtils.updateElement.call(this, id, content);
        }

        Osiris.ajax.AjaxUtils.handleResponse.call(this, xmlDoc);
    }
    ...
    Osiris.ajax.Queue = {
        ...
        offer : function(request) {
            ...
            if(this.requests.length == 1) {
                Osiris.ajax.AjaxUtils.send(request);
            }
        },
        ...
        poll : function() {
            ...
            if(next != null) {
                Osiris.ajax.AjaxUtils.send(next);
            }
    ...
    /**
     * BaseWidget for Osiris Widgets
     */
    Osiris.widget.BaseWidget = Class.extend({

        init: function(cfg) {
            this.cfg = cfg;
            this.id = cfg.id;
            this.jqId = Osiris.escapeClientId(this.id);
            this.jq = $(this.jqId);
        ...
        removeScriptElement: function(clientId) {
        	$(Osiris.escapeClientId(clientId) + '_s').remove();
        }
    });
